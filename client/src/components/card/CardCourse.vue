    ...
    <div href="#" class="relative block overflow-hidden rounded-lg border border-gray-100 p-4 sm:p-6 lg:p-8">
      ...
      <dl class="mt-6 grid grid-cols-3 gap-4 sm:gap-6 mb-6">
        ...
        <div class="flex flex-col-reverse">
          ...
          <dd class="text-xs text-gray-500">{{ course.Mentor ? course.Mentor.name : "" }}</dd>
        </div>
      </dl>
    </div>
    <button @click="getDetail" class="absolute inset-x-0 bottom-0 mt-3 bg-gradient-to-r text-white from-green-300 via-blue-500 to-purple-600 rounded-b-lg text-center py-2">Detail</button>
  </div>
</template>
